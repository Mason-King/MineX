...
public class MobSpawn {
    ...
    public void spawn() {
        for(int i = 0; i < ThreadLocalRandom.current().nextInt(minMobs, maxMobs); i++) {
            String entityType = entities.get(ThreadLocalRandom.current().nextInt(0, entities.size()));
            Location loc = Utils.fromString(location);
            loc.getWorld().spawnEntity(loc,EntityType.fromName(entityType));
        }
    }
